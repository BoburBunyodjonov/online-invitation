...
const DEFAULT_MUSIC = `${ASSETS}/skripka%20music.mp3`;
const MUSIC_VOLUME = 0.16;
const OPENING_DURATION_MS = 1000;
const LANGUAGE_STORAGE_KEY = "blueEnvelopeLanguage";
...
function getInitialLang(data: InvitationData): BlueEnvelopeLang {
  ...
  return toBlueEnvelopeLang(data.defaultLocale);
}
...
function BlueEnvelopeInner({ data }: { data: InvitationData }) {
  const [lang, setLang] = useState<BlueEnvelopeLang>(() =>
    toBlueEnvelopeLang(data.defaultLocale),
  );
  ...
  const ornamentNamesRef = useRef<HTMLParagraphElement>(null);
  const fitFrameRef = useRef<number | null>(null);

  const ui = BLUE_ENVELOPE_UI[lang];
  ...
  const ornamentDate = formatOrnamentDateParts(data.weddingDate);
  ...
  const heroHtml = data.greeting?.[lang] || data.greeting?.ru
    ? textToHtml(pickLocalized(data.greeting, lang, ui.heroNames))
    : ui.heroNames;

  const leadHtml = data.intro?.[lang] || data.intro?.ru
    ? textToHtml(pickLocalized(data.intro, lang, ui.lead))
    : ui.lead;

  const ornamentMessageHtml = (() => {
    const translation = pickLocalized(data.verse?.translation, lang);
    const source = pickLocalized(data.verse?.source, lang);
    if (translation) {
      return source
        ? `${textToHtml(translation)}<br />(${source})`
        : textToHtml(translation);
    }
    return lang === "uz"
  ...
  const fitOrnamentNames = useCallback(() => {
    const namesBlock = ornamentNamesRef.current;
  ...
  const scheduleOrnamentNameFit = useCallback(() => {
  ...
  const revealVisibleSections = useCallback(() => {
    const reveals = document.querySelectorAll(".blue-envelope-root .reveal");
  ...
  }, [isOpening, playMusic, revealVisibleSections, scheduleOrnamentNameFit]);

  const handleLanguageChange = (next: BlueEnvelopeLang) => {
    if (next === lang) return;
    setLang(next);
    try {
  ...
  }, [data.unlockGate, revealVisibleSections, scheduleOrnamentNameFit]);
  ...
  useEffect(() => {
    ...
    if (document.fonts?.ready) {
      document.fonts.ready.then(scheduleOrnamentNameFit);
    }
    return () => {
      window.removeEventListener("resize", scheduleOrnamentNameFit);
      if (fitFrameRef.current !== null) {
        window.cancelAnimationFrame(fitFrameRef.current);
      }
    };
  }, [lang, invitationVisible, scheduleOrnamentNameFit]);
  ...
  return (
    <div className={rootClass}>
      ...
      <main className="invitation" aria-hidden={!invitationVisible}>
        <section className="letter-hero reveal" id="letterHero">
          ...
          >
            <div className="ornament-content">
              <p className="ornament-names" ref={ornamentNamesRef}>
              ...
              <p
                className="ornament-message"
                dangerouslySetInnerHTML={{ __html: ornamentMessageHtml }}
              />

              <div className="ornament-date" aria-hidden>
                <span>{ornamentDate.day}</span>
                <i />
                <span>{ornamentDate.month}</span>
                <i />
                <span>{ornamentDate.year}</span>
              </div>
            </div>
          </article>
        ...
        <section className="letter-card reveal">
          <h1
...
export default function BlueEnvelopeTemplate({ data }: TemplateProps) {
  return <BlueEnvelopeInner data={data} />;
}
